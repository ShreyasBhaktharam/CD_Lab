...
symbol* allocate_space_for_symbol(char* name, int size, int type, int lineno, int scope);
//allocates space for items in the list and initialisation

void insert_into_table(table* t_name, symbol*);
...
int check_symbol_table(table* t_name, char*);
//checks symbol table whether the variable has been declared or not

void display_symbol_table(table*);			//displays symbol table
