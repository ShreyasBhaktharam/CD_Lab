...
static const yytype_uint8 yyrline[] =
{
      ...
      47,    64,    85,    86,    87,    88,    92,   113,   114,   117,
     123,   130,   134,   140,   146,   149,   150,   166,   174,   183,
     184,   185,   186,   187,   188,   193,   193,   195,   196,   199,
     200,   201,   205,   206,   209,   209
};
#endif
...
#line 47 "parser.y"
			...
			if(check_symbol_table(t, yyvsp[-2])){
	// 					// symbol exists. raise error
				...
				int type = get_type(yyvsp[0]);

				symbol* new_symbol = allocate_space_for_symbol(yyvsp[-2], size, CUR_TYPE, yylineno, scope);
				insert_into_table(t, new_symbol);
				insert_value_to_name(t, yyvsp[-2], yyvsp[0]);
			}
		}
#line 1451 "y.tab.c"
    break;
...
#line 64 "parser.y"
				...
				*/
					...
					if(!check_symbol_table(t, yyvsp[0])){
						symbol* new_symbol = allocate_space_for_symbol(yyvsp[0], size, CUR_TYPE, yylineno, scope);
						insert_into_table(t, new_symbol);
					}
					else {
...
#line 92 "parser.y"
				...
				if(check_symbol_table(t, yyvsp[-2])){
					// symbol exists.
					int ltype = get_symbol(t, yyvsp[-2])->type;
					int rtype = get_type(yyvsp[0]);
...
#line 117 "parser.y"
              {
			if(CUR_TYPE == INT)
				sprintf(yyval, "%d", atoi(yyvsp[0]) + atoi(yyvsp[-2]));
			else
				sprintf(yyval, "%f", atof(yyvsp[0]) + atof(yyvsp[-2]));
		}
#line 1540 "y.tab.c"
    break;
...
#line 150 "parser.y"
           {
			if(!check_symbol_table(t, yyvsp[0])) {
				...
				symbol* sym = get_symbol(t, yyvsp[0]);
				yyval = strdup(sym->val);
...
#line 166 "parser.y"
            {
			yyval = strdup(yyvsp[0]);
			if(!is_valid_type(CUR_TYPE, get_type(yyvsp[0]))){
				printf("Mismatch type\n");
				yyerror(yyvsp[0]);
			}
		}
